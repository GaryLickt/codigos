...
if (oAllan.dead = false) {
	...
	if oAllan.visi = true {
		...
		if !audio_is_playing(snd_enquanto_usafrag) {
		audio_play_sound(snd_enquanto_usafrag,1,true);
		}
	}else{
	if audio_is_playing(snd_enquanto_usafrag) {
		audio_stop_sound(snd_enquanto_usafrag);
		}
	}
	...
	if (room_get_name(room) == "Room4") and (cena1 = true) {
